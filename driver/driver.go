...
func (d *Driver) DisconnectDevice(deviceName string, protocols map[string]models.ProtocolProperties) error {
	d.lc.Info(fmt.Sprintf("Driver.DisconnectDevice: device-lte-go driver is disconnecting to %s", deviceName))
	return nil
}
...
func (d *Driver) HandleReadCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []dsModels.CommandRequest) (res []*dsModels.CommandValue, err error) {
	...
	if reqs[0].DeviceResourceName == "Value" {
		// 发送数据
		value := []byte(send)
		_, err = client.Write(value)
		if err != nil {
		...
		cv, _ := dsModels.NewInt64Value(reqs[0].DeviceResourceName, now, int64(num))
		res[0] = cv
	} else{
...
func (d *Driver) HandleWriteCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []dsModels.CommandRequest,
	...
	defer client.Close()
	if params[0].DeviceResourceName == "Light" {
		send := "Light"
		data = []byte(send)
...
func (d *Driver) Stop(force bool) error {
	d.lc.Info("Driver.Stop: device-lte-go driver is stopping...")
	return nil
}
